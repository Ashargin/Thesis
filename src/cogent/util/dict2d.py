...
def average(upper, lower):
    ...
    try:
        ...
        raise TypeError(
            "%s or %s invalid types for averaging." % (str(upper), str(lower))
        )
...
class Dict2D(dict):
    ...
    _copied_attributes = ["RowOrder", "ColOrder", "Default", "Pad", "RowConstructor"]
    ...
    def _guess_input_type(self, data):
        ...
        if isinstance(data, dict):
            ...
            try:
                ...
                if (
                    (RowOrder is not None)
                    and (ColOrder is not None)
                    and (len(data) == len(RowOrder))
                    and (len(data[0]) == len(ColOrder))
                ):
    ...
    def _get_cols(self):
        ...
        if self.Pad:
            ...
            constructor = self.RowConstructor
            for c in col_order:
                yield [self.get(r, constructor()).get(c, default) for r in row_order]
        else:
            try:
                for c in col_order:
    ...
    def getRowIndices(self, f, negate=False):
        ...
        return [key for key, row in zip(row_order, self.Rows) if new_f(row)]
    ...
    def getCols(self, cols, negate=False, row_constructor=None):
        ...
        if negate:
            ...
            for key, row in self.items():
                result[key] = row_constructor(
                    [(i, row[i]) for i in row if (i in row) and (i not in col_lookup)]
                )
        # otherwise, just get the requested indices
        else:
            for key, row in self.items():
                result[key] = row_constructor([(i, row[i]) for i in cols if i in row])
        return self.__class__(result)
    ...
    def getColIndices(self, f, negate=False):
        ...
        return [
            i
            for i, col in zip(self.ColOrder or self.colKeys(), self.Cols)
            if new_f(col)
        ]
    ...
    def getColsIf(self, f, negate=False, row_constructor=None):
        ...
        if row_constructor is None:
            row_constructor = self.RowConstructor
        return self.getCols(
            self.getColIndices(f, negate), row_constructor=row_constructor
        )
    ...
    def getItems(self, items, negate=False):
        ...
        # otherwise, just pick the selected items out of the list
            ...
            if self.Pad:
                ...
                default = self.Default
                return [
                    self.get(row, row_constructor()).get(col, default)
    ...
    def reflect(self, method=average):
        ...
        for row_index in range(len(row_order)):
            ...
            for col_index in range(row_index):
                ...
                # as written.
                curr_row[c], curr_col[r] = method(
                    curr_col.get(r, default), curr_row.get(c, default)
                )

    def toDelimited(
        self, headers=True, item_delimiter="\t", row_delimiter="\n", formatter=str
    ):
        ...
        return row_delimiter.join(
            [item_delimiter.join(map(formatter, r)) for r in lists]
        )
