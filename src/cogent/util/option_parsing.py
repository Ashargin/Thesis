...
__credits__ = ["Greg Caporaso", "Daniel McDonald", "Gavin Huttley", "Rob Knight"]
__license__ = "GPL"
__version__ = "1.9"
...
def check_existing_filepath(option, opt, value):
    if not exists(value):
        raise OptionValueError("option %s: file does not exist: %r" % (opt, value))
...
def check_existing_dirpath(option, opt, value):
    if not exists(value):
        raise OptionValueError("option %s: directory does not exist: %r" % (opt, value))
...
def check_existing_path(option, opt, value):
    if not exists(value):
        raise OptionValueError("option %s: path does not exist: %r" % (opt, value))
    return value
...
def build_usage_lines(
    ...
    line1 = "usage: %prog [options] " + "{%s}" % " ".join(
        ["%s %s" % (str(ro), ro.dest.upper()) for ro in required_options]
    )
    usage_examples = []
...
def parse_command_line_parameters(**kwargs):
    ...
    disallow_positional_arguments = set_parameter(
        "disallow_positional_arguments", kwargs, True
    )
    help_on_no_arguments = set_parameter("help_on_no_arguments", kwargs, True)
    optional_input_line = set_parameter(
        "optional_input_line", kwargs, "[] indicates optional input (order unimportant)"
    )
    required_input_line = set_parameter(
        "required_input_line", kwargs, "{} indicates required input (order unimportant)"
    )
    ...
    if disallow_positional_arguments and len(args) != 0:
        parser.error(
            "Positional argument detected: %s\n" % str(args[0])
    ...
    if required_options:
        ...
        for required_option_id in required_option_ids:
            if getattr(opts, required_option_id) == None:
                return parser.error(
                    "Required option --%s omitted." % required_option_id
                )
