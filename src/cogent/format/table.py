...
def simpleFormat(
    ...
    if len(space.join(header)) > max_width:
        ...
        for i in range(begin, len(header)):
            ...
            if width > max_width:
                subtable_boundaries.append((begin, i, width - col_widths[i] - sep))
        ...
        for start, end, width in subtable_boundaries:
            ...
            subhead = space.join(
                [space.join(header[:identifiers]), space.join(header[start:end])]
            )
            width = len(subhead)
            table.append("=" * width)
            ...
            for row in formatted_table:
                row = [space.join(row[:identifiers]), space.join(row[start:end])]
