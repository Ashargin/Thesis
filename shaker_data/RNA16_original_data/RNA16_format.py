...
parser.add_argument(
    "--bpseq", type=str, default="BPseq", help="path to folder containing .bpseq files"
)
parser.add_argument(
    "--seq", type=str, default="seq", help="path to folder containing .seq files"
)
parser.add_argument(
    "--shape", type=str, default="shape", help="path to folder containing .shape files "
)
...
def dump_fasta(se, dp, stringname):
    ...
    textlist = [
        ">%s\n%s\n%s\n\n" % (s.strip(), d.strip(), n.strip())
        for (s, d, n) in zip(stringname, se, dp)
    ]
    res = "".join(textlist)
    dump("data.dbn", res)
    print(res)
